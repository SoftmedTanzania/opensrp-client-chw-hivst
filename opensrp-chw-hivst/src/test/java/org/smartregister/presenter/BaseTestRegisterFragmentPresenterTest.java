...
public class BaseTestRegisterFragmentPresenterTest {
    ...
    public void getDueFilterCondition() {
        Assert.assertEquals(" (cast( julianday(STRFTIME('%Y-%m-%d', datetime('now'))) -  julianday(IFNULL(SUBSTR(hivst_test_date,7,4)|| '-' || SUBSTR(hivst_test_date,4,2) || '-' || SUBSTR(hivst_test_date,1,2),'')) as integer) between 7 and 14) ", baseTestRegisterFragmentPresenter.getDueFilterCondition());
    }

    @Test
    public void getDefaultSortQuery() {
        Assert.assertEquals(Constants.TABLES.HIVST_CONFIRMATION + "." + DBConstants.KEY.LAST_INTERACTED_WITH + " DESC ", baseTestRegisterFragmentPresenter.getDefaultSortQuery());
    }

    @Test
    public void getMainTable() {
        Assert.assertEquals(Constants.TABLES.HIVST_CONFIRMATION, baseTestRegisterFragmentPresenter.getMainTable());
    }
