...
public class HivstUtil {

    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            HivstJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(HivstJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static ECSyncHelper getSyncHelper() {
        return HivstLibrary.getInstance().getEcSyncHelper();
    }

    public static ClientProcessorForJava getClientProcessorForJava() {
        return HivstLibrary.getInstance().getClientProcessorForJava();
    }
    ...
    public static boolean launchDialer(final Activity activity, final BaseHivstCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> HivstUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static class CloseHivstMemberFromRegister extends AsyncTask<Void, Void, Void> {
        ...
        public CloseHivstMemberFromRegister(String baseEntityId) {
            this.baseEntityId = baseEntityId;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            HivstDao.closeHivstMemberFromRegister(baseEntityId);
            return null;
        }
